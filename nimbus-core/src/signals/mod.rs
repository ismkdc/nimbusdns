...
mod imp {
    ...
    /// On Windows, Unix signals are not available. We register a CTRL+C handler
    /// via tokio and expose a placeholder reload channel (no-op).
    pub fn setup(
        shutdown_tx: watch::Sender<bool>,
        _reload_tx: watch::Sender<bool>,
    ) -> impl futures_util::Future<Output = ()> {
        let fut = async move {
            info!("Signal handlers initialized (Windows: CTRL+C only)");
            tokio::select! {
